...
describe Comment do
  context "when a comment is created" do
    ...
    it "returns validation error if body is missing" do
      expect( FactoryBot.build(:comment, product: product, body: nil, user: user) ).not_to be_valid
    end

    it "returns validation error if rating is missing" do
      expect( FactoryBot.build(:comment, product: product, rating: nil, user: user) ).not_to be_valid
    end
    ...
    it "returns validation error if user has already reviewed the same product" do
      ...
      expect( FactoryBot.create(:comment, product: product, user: user) ).not_to be_valid
    end
  end
end